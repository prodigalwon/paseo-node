...
use super::{
	...
	SLOT_DURATION, UNIT, MILLI_UNIT, VERSION,
};
...
// ════════════════════════════════════════════════════════════
// Consensus: Aura + Grandpa (solochain)
// ════════════════════════════════════════════════════════════
...
/// Reads the Aura slot from pre-runtime digests, indexes into the authority
/// list, and converts the AuraId (sr25519::Public) to AccountId32.
/// SecretSquirrel uses this to pay the block author on slot cleanup.
...
// ════════════════════════════════════════════════════════════
// Insecure Randomness (TESTNET ONLY)
// ════════════════════════════════════════════════════════════

// WARNING: pallet_insecure_randomness_collective_flip is NOT safe for production.
// Validators can manipulate TTL, violating the "sender does not know the TTL"
// invariant. For mainnet, wire relay-chain randomness via cumulus.
...
impl pallet_secret_squirrel::Config for Runtime {
    type RuntimeCall = RuntimeCall;
    type Currency = Balances;
    /// TESTNET: CollectiveFlip. Production must use relay-chain randomness.
    type BabeRandomness = super::InsecureRandomness;
