//! Parachain collator service: cumulus Aura (lookahead) authoring +
//! relay chain connectivity via RPC + PNS / ZK-PKI / SecretSquirrel
//! runtime APIs surfaced through `crate::rpc`.
//!
//! Structure mirrors `polkadot-sdk/templates/parachain/node/src/service.rs`
//! at SHA `241bd90` (stable2603). Finality inherited from the relay
//! chain — no GRANDPA.
...
use paseo_runtime::{apis::RuntimeApi, opaque::Block};
...
pub(crate) type ParachainExecutor = WasmExecutor<ParachainHostFunctions>;
pub(crate) type FullClient = TFullClient<Block, RuntimeApi, ParachainExecutor>;
...
pub async fn start_parachain_node(
	...
	// Para ID sourced from runtime (`GetParachainInfo`), NOT the chain
	// spec extension. Spec extension's `para_id` is an advisory hint
	// for tooling.
	let best_hash = client.chain_info().best_hash;
	let para_id = client
		.runtime_api()
		.parachain_id(best_hash)
		.map_err(|_| "Failed to retrieve parachain id from runtime. Ensure `GetParachainInfo` runtime API is implemented.")?;
	...
	let (network, system_rpc_tx, tx_handler_controller, sync_service) =
		build_network(BuildNetworkParams {
			...
			metrics: sc_network::NetworkWorker::<Block, <Block as sp_runtime::traits::Block>::Hash>::register_notification_metrics(
	...
	if let Some(hwbench) = hwbench {
		...
		match SUBSTRATE_REFERENCE_HARDWARE.check_hardware(&hwbench, false) {
			Err(err) if validator => {
				log::warn!(
					"⚠️  Hardware does not meet minimal requirements {} for role 'Authority'.",
					err,
				);
			},
			_ => {},
	...
	if validator {
		start_consensus(
			...
			relay_chain_slot_duration,
			para_id,
			collator_key.expect("Command line args forbid this. qed"),
			collator_peer_id,
			overseer_handle,
			announce_block,
		)?;
